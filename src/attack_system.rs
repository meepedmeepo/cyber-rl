...
use super::{Attack, Name, State, TakeDamage};
...
impl AttackSystem {
    pub fn add_attack(attacker: hecs::Entity, target: hecs::Entity, state: &mut State) {
        ...
        state
            .world
            .insert_one(attacker, Attack { target: target })
    ...
    pub fn run(state: &mut State) {
        ...
        for (_id, (attack, _name, stats, pos)) in state
            .world
            .query::<(&mut Attack, &Name, &BaseStatistics, &Position)>()
            .iter()
        {
        ...
        for (target, attacker, atkstats) in defenders_to_damage {
            let query = state
                .world
                .query_one_mut::<(&BaseStatistics, &Pools)>(target)
                .expect("");
            ...
            if weapons.len() < 1 {
                ...
                if attacker
                    != state
                        .player_ent
                        .expect("Couldn't find player to check if it is them who is attacking!")
                {
                    let nat_weps = state
                        .world
                        .query_one_mut::<&Naturals>(attacker)
                        .expect("MONSTERS SHOULD HAVE NATURALS EVEN IF LIST IS EMPTY")
                        .weapons
                        .clone();
                    if nat_weps.len() > 0 {
                        found_wep = true;
                ...
                if !found_wep {
                    weapons.push(Weapon {
                        uses_statistic: WeaponStat::Strength,
                        damage_die: 4,
                        dmg_bonus: 2,
                        to_hit_bonus: 0,
                        num_dmg_dice: 1,
            ...
            if hit {
                ...
                add_effect(
                    Some(attacker),
                    crate::effects::EffectType::Damage { amount: dmg },
