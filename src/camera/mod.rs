...
pub fn render_camera(state: &mut State) {
        ...
        for tx in min_x..max_x {
            if tx > 0 && tx < map_width && ty > 0 && ty < map_height {
                ...
                if state.map.revealed_tiles[idx] {
                    ...
                    state
                        .renderer
                        .draw_char_bg(x, y, cont, rgb_to_color(fg), rgb_to_color(bg));
                }
            } else if SHOW_BOUNDARIES {
                state.renderer.draw_char_bg(
                    x,
                    y,
                    *".".to_cp437(&CP437_WINGDINGS).unwrap().first().unwrap(),
                    rgb_to_color(RGB::named(GRAY)),
    ...
    let mut entities_to_render = state
        .world
    ...
    for ent in entities_to_render {
        ...
        if state.map.visible_tiles[idx] {
            ...
            if entity_screen_x > 0
                && entity_screen_x < max_x - 1
                && entity_screen_y > 0
                && entity_screen_y < max_y - 1
            {
                ...
                let cont = *ent
                    .1
                    .glyph
                    .to_cp437(&CP437_WINGDINGS)
                    .unwrap()
                    .first()
                    .unwrap();
                state
                    .renderer
                    .draw_char_bg(entity_screen_x, entity_screen_y, cont, fg, bg);
            }
        }
    }
}
