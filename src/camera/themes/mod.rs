use bracket_lib::{color::{DARKSLATEGRAY, GHOST_WHITE, GRAY, GREY40, LIGHT_GRAY, RGB}, prelude::{to_cp437, FontCharType}};
...
fn get_tile_glyph_default(idx : usize, map : &Map) -> (FontCharType, RGB, RGB)
{
    ...
    match map.map[idx]
    {
        ...
        TileType::Concrete => {glyph = to_cp437('░'); fg = RGB::named(GRAY);}
        TileType::Footpath => {glyph = to_cp437('▒'); fg = RGB::named(GHOST_WHITE);}
