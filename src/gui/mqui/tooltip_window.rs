...
use crate::{camera, components::Name, map_indexing::SPATIAL_INDEX, State};
...
pub fn show_tooltip_window(ctx: &egui::Context, state: &mut State, x: i32, y: i32) {
    ...
    let entities = SPATIAL_INDEX
        .lock()
        .unwrap()
        .get_tile_contents(state.map.xy_idx(x, y));
    ...
    if name.len() > 0 {
        let (screen_x, screen_y) = state
            .renderer
            .canvas
            .get_tile_screen_pos(x - min_x, y - min_y);
