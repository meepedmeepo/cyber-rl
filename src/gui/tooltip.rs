...
use crate::{camera, map_indexing::SPATIAL_INDEX, Map, Name, Renderable, State};

pub fn draw_tooltip(state: &mut State, cursor_pos: Point) {
    ...
    if cursor_map_pos.x >= state.map.map_width - 1
        || cursor_map_pos.y >= state.map.map_height - 1
        || cursor_map_pos.x < 1
        || cursor_map_pos.y < 1
    {
        return;
    ...
    let content_id = SPATIAL_INDEX.lock().unwrap().get_tile_contents(idx);
