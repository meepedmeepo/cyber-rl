...
pub fn draw_status_box(state: &mut State) {
    ...
    for (_id, (_player, stats, bstat, hunger)) in
        state
            .world
            .query_mut::<(&Player, &Pools, &BaseStatistics, &HungerLevel)>()
    {
        let health = format!(
            "HP: {} / {} ",
            stats.hitpoints.current_value, stats.hitpoints.max_value
        );
...
pub fn draw_cursor(pos: Point, state: &mut State, bg: (u8, u8, u8)) -> Point {
    ...
    state.renderer.draw_square(
        cursor_pos.x,
        cursor_pos.y,
        color_with_alpha(rgb_to_color(col), 0.4),
    );
