...
pub fn find_entity_spawn_locations(build_data: &BuilderMap) -> HashSet<usize> {
    ...
    build_data
        ...
        .filter_map(|(b, _a)| {
            ...
            if !SPATIAL_INDEX.lock().unwrap().is_tile_blocked(b) && !used_locations.contains(&b) {
                Some(b)
            } else {
                None
            }
...
pub fn spawn_building_contents(
    build_data: &mut BuilderMap,
    contents: &[String],
    building_tiles: &mut HashSet<usize>,
) {
...
fn road_get_candidates(
    build_data: &BuilderMap,
    roads: &mut GraphMap<usize, i32, petgraph::Undirected>,
) {
    let w = build_data.map.map_width;
