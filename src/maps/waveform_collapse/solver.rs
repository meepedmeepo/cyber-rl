...
pub struct Solver
{
    ...
    chunks_x : usize,
    chunks_y: usize,
    remaining: Vec<(usize, i32)>,//first element is index, 2nd is number of neighbours
    pub possible: bool
}
...
impl Solver
{
    ...
    pub fn iteration(&mut self, map : &mut Map, rng : &mut RandomNumberGenerator) -> bool
    {
        ...
        //pick a random chunk that hasn't been touched and remove it from remaining whilst fetching the index
        ...
        if neighbors == 0
        {
            //Nothing nearby so anything is an option
        ...
        else 
        {
            ...
            {
                console::log("WFC Solution not possible!");
                self.possible = false;
                return true;
            } else {
