...
impl NetworkMap
{
    pub fn get_connections(&self, ent : Entity) -> Option<&Vec<NodeConnection>>
    {
        self.graph.iter()
            ...
            .and_then(|(index,edges)| { return Some(edges);} )
    }
    ...
    {
        self.graph.iter_mut().find(|(index, edges)| **index == NodeIndex{entity: node1})
        .map_or( false, |(index, edges)| 
        {
            edges.push(NodeConnection{entity: node2}); return true
        })
    }
