use bracket_lib::{
    color::{BLACK, BLUE, RED, RGB, WHITE, YELLOW},
    prelude::{field_of_view, BTerm, Point, VirtualKeyCode},
};
use codepage_437::{ToCp437, CP437_CONTROL};
use hecs::Entity;
use macroquad::{
    color::GREEN,
    input::{is_key_down, is_key_pressed, is_mouse_button_down, KeyCode, MouseButton},
};

use crate::{
    camera,
    gui::{self, TargettingMode},
    raws::{faction_reaction, Reaction, RAWS},
    renderer::color_with_alpha,
    utils::get_mob_entities_at_position,
    Faction, FoV, Hidden, Map, Position, State,
};
...
pub fn aim_projectile(state: &mut State, start_pos: Point, range: i32) -> TargettingState {
    ...
    if is_key_pressed(KeyCode::Tab) {
        if let TargettingMode::Keyboard { cursor_pos } = state.target_mode {
            state.target_mode = TargettingMode::Keyboard {
                cursor_pos: select_nearest_target_pos(state, state.player_ent.unwrap(), cursor_pos),
            }
        }
    }
    ...
    if available_cells.contains(&point) {
        ...
        _line.skip(1).for_each(|pos| {
            ...
            state.renderer.draw_char_bg(
                screen_pos.x,
                screen_pos.y,
                *"*".to_cp437(&CP437_CONTROL).unwrap().first().unwrap(),
                macroquad::prelude::BLACK,
                macroquad::prelude::GREEN,
            );
        });

        state.renderer.draw_square(
            screen_point.x,
            screen_point.y,
            color_with_alpha(macroquad::prelude::GREEN, 0.4),
        );

        if is_mouse_button_down(MouseButton::Left) {
            let targets = bracket_lib::geometry::Bresenham::new(start_pos, point).collect();
            return TargettingState::Selected {
                path: targets,
                end: point,
            };
        }
        ...
        state.renderer.draw_square(
            screen_point.x,
            screen_point.y,
            color_with_alpha(macroquad::prelude::RED, 0.4),
        );
...
fn targetting_viewshed(
    valid_tiles: &mut Vec<Point>,
    state: &mut State,
    range: i32,
    pos: Point,
    should_display: bool,
            ...
            if should_display {
                    ...
                    && screen_y < (max_y - min_y) - 1
                {
                    state.renderer.draw_square(
                        idx.x - min_x,
                        idx.y - min_y,
                        color_with_alpha(macroquad::prelude::BLUE, 0.4),
                    );
                }
            }
...
pub fn select_nearest_target_pos(state: &mut State, ent: Entity, current_pos: Point) -> Point {
    ...
    for i in fov.iter() {
        let cont = get_mob_entities_at_position(state, *i);
        for mob in cont.iter() {
            ...
            if react == Reaction::Attack
                && their_point != current_pos
                && (closest == Point::zero()
                    || bracket_lib::pathfinding::DistanceAlg::Pythagoras
                        .distance2d(point, their_point)
                        < bracket_lib::pathfinding::DistanceAlg::Pythagoras
                            .distance2d(point, closest))
            {
                closest = their_point;
            }
        }
