...
use bracket_lib::{
    color::RGB,
    prelude::to_cp437,
    random::{parse_dice_string, DiceType},
};
...
impl RawMaster {
    ...
    fn add_renderable_comp(
        ...
        eb.add(components::Renderable {
            ...
            bg: bracket_lib::color::RGB::from_hex(&renderable.bg)
                .expect(format!("Invalid RGB {}", renderable.glyph.clone()).as_str()),
    ...
    pub fn add_effects_comps(
        ...
        for effect in effects.iter() {
            ...
            match effect_name {
                "provides_healing" => {
                    eb.add(HealingEffect {
                        healing_amount: effect.1.parse::<i32>().unwrap(),
                    });
                }
                "ranged" => {
                    eb.add(RangedTargetting {
                        range: effect.1.parse::<i32>().unwrap(),
                    });
                }
                "damage" => {
                    eb.add(DamageEffect {
                        damage_amount: effect.1.parse::<i32>().unwrap(),
                    });
                }
                "aoe" => {
                    eb.add(AoE {
                ...
                _ => {
                    bracket_lib::terminal::console::log(format!(
                        "Warning: effect {} not implemented.",
                        effect_name
                    ));
                }
            }
    ...
    pub fn parse_particle_string(particle_string: String) -> Particle {
        ...
        let lifetime = parts[3]
            .parse::<f32>()
            .expect("not valid f32 for particle lifetime");
    ...
    fn parse_weapon_comp(weapon: super::Weapon) -> components::Weapon {
        ...
        match slotname {
            "strength" => {
                stat = WeaponStat::Strength;
            }
            "dexterity" => {
                stat = WeaponStat::Dexterity;
            }
            _ => {
                panic!("error that wasn't a valid weapon statistic!");
            }
        }

        components::Weapon {
            uses_statistic: stat,
            damage_die: dmg_die.die_type,
            to_hit_bonus: weapon.to_hit_bonus,
            dmg_bonus: dmg_die.bonus,
            num_dmg_dice: dmg_die.n_dice,
        }
    }
    ...
    fn add_monster_stats_comp(new_entity: EntityBuilder, stats: &MobStats) -> EntityBuilder {
        ...
        eb.add(Pools {
            hitpoints: StatPool::new(stats.max_hp),
            exp: 0,
            level: level,
            armour_class: Attribute::new(ac),
            hit_die: DiceType::new(1, 6, 0),
        });

        eb
    }

    pub fn spawn_named_mob<'a>(
        raws: &'a RawMaster,
        new_entity: hecs::EntityBuilder,
        key: &str,
        pos: SpawnType,
    ...
    pub fn spawn_named_item<'a>(
        ...
        if raws.item_index.contains_key(key) {
            ...
            if let Some(range) = &item_template.ranged {
                eb.add(RangedWeapon {
                    range: range.range,
                    damage: parse_dice_string(&range.damage)
            ...
            if let Some(equipment) = &item_template.equippable {
                ...
                match slotname {
                    ...
                    _ => {
                        panic!("Equipment slot incorrect in json!");
                    }
                }
                eb.add(Equippable { slot: slot });
            }
            ...
            if let Some(proj) = &item_template.rangedprojectile {
                eb.add(ParticleAnimation {
                    particle: RawMaster::parse_particle_string(proj.clone()),
                });
            }
    ...
    pub fn spawn_named_prop<'a>(
        ...
        if raws.prop_index.contains_key(key) {
            ...
            //todo! add options for interactable to not have an effect spawner mayhaps?
            if let Some(interactable) = &prop_template.interactable {
                let prefab = interactable.consumable.as_ref().unwrap().effects.clone();

                eb.add(EffectSpawnerPrefab { prefab });
            }
    ...
    pub fn spawn_effect_spawner<'a>(
        _raws: &'a RawMaster,
        new_entity: hecs::EntityBuilder,
        parent_entity: Entity,
        prefab: EffectSpawnerPrefab,
    ) -> Box<EntityBuilder> {
        let mut eb = new_entity;

        eb = RawMaster::add_effects_comps(eb, prefab.prefab);
        eb.add(EffectSpawner {
            owner: parent_entity,
        });
