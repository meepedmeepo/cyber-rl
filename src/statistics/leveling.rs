...
pub fn check_level_up(state: &mut State) {
    let (stats, pools) = state
        .world
        .query_one_mut::<(&BaseStatistics, &mut Pools)>(
            state
                .player_ent
                .expect("Couldn't find player to check level!"),
        )
        .unwrap();
    ...
    if xp_to_next_level(pools.level, pools.exp) <= 0 {
        ...
        state.particle_builder.request(
            state.player_pos.x,
            state.player_pos.y,
            RGB::named(WHITE),
            RGB::named(ORANGE),
...
pub fn get_xp_from_current_level(state: &mut State) -> (i32, i32) {
    let pools = state
        .world
        .query_one_mut::<&Pools>(state.player_ent.unwrap())
        .unwrap();
